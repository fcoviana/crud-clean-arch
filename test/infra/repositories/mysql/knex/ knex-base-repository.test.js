const KnexBaseRepository = require('../../../../../src/infra/repositories/mysql/knex/knex-base-repository');
const { NotFoundError } = require('../../../../../src/shared/utils/errors');
const EntitySpy = require('../../../../mocks/entity');
...
const makeUpTable = async (conn) => {
  return conn.schema.createTable('entities', (table) => {
...
const makeDownTable = (conn) => conn.schema.dropTable('entities');

const makeSut = () => {
  const entityData = makeEntityData();

  const sut = new KnexBaseRepository(EntitySpy, 'entities');
...
describe("repository: KnexBaseRepository", () => {
  ...
  test("Should instantiate a repository without a context and create", async () => {
    const sut = new KnexBaseRepository();

    await expect(sut.create()).rejects.toThrow();
    await expect(sut.create()).rejects.toThrow(TypeError);
    await expect(sut.create()).rejects.toThrow("this.entity is not a constructor");
  });
  ...
  test("Should return NotFoundError fetchOne", async () => {
    ...
    await expect(sut.fetchOne(id)).rejects.toThrow("EntitySpy not found!");
  });
  ...
  test("Should instantiate a repository without a context and fetchAll", async () => {
    const sut = new KnexBaseRepository();

    await expect(sut.fetchAll()).rejects.toThrow();
  });
  ...
  test("Should instantiate a repository without a context and update", async () => {
    ...
    await expect(sut.update()).rejects.toThrow("this.entity is not a constructo");
  });
  ...
  test("Should instantiate a repository without a context and destroy", async () => {
    const sut = new KnexBaseRepository();
  ...
  test("Should call destroy without parameter", async () => {
    const { sut } = makeSut();
  ...
  test("Should instantiate a repository without a context and delete", async () => {
    ...
    await expect(sut.delete()).rejects.toThrow("Undefined binding(s) detected when compiling DEL. Undefined column(s): [id] query: delete from  where `id` = ?");
  });
  ...
  test("Should call delete without parameter", async () => {
    ...
    await expect(sut.delete()).rejects.toThrow();
    await expect(sut.delete()).rejects.toThrow(Error);
    await expect(sut.delete()).rejects.toThrow("Undefined binding(s) detected when compiling DEL. Undefined column(s): [id] query: delete from `entities` where `id` = ?");
  });
});
